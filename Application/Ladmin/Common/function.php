 ...
 * 检查短信是否开启
 */
function is_sms_open(){
    $is = M('sms')->getField('is_open');
 ...
 * 检查邮箱是否配置
 */
function is_mail_open(){
    $is = M('mail')->getField('is_open');
    return $is == 1 ? true : false;
}
 ...
 * 发送邮件
 */
function mailSend($title, $toMail, $product, $config){
    ...
    $mail->Password = $config['mail_pass'];               // 邮箱授权码，注意是授权码，不是登录密码
    $mail->SMTPSecure = 'ssl';                            // 目前规定必须使用ssl，非ssl的协议已经不支持了
    $mail->Port = $config['mail_port'];                   // 端口号
    $mail->From = $config['mail_user'];                   // 发件人地址（也就是你的邮箱地址）
    $mail->FromName = $config['mail_name'];               // 发件人姓名
    $mail->addAddress($toMail);                           // 收件人地址
    $mail->isHTML(true);                           // Set email format to HTML
    $mail->Subject = $title;
    $mail->Body    = $product;
//    $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';    //备用
    return $mail->send() ? true : $mail->ErrorInfo;
}

/**
 * 检查当天短信发送次数并判断手机号码是否存在
 */
function checkSend($mobile,$max_smsNum){
    ...
    if(file_exists($fileName)){//文件存在获取次数并将此次请求的数据写入
        ...
        $LoginN = $countArr[$mobile];
        if($LoginN<=$max_smsNum)
        {
            file_put_contents($fileName,$mobile."|",FILE_APPEND);
        }else{
            $LoginN = 999;
        }
        return $LoginN;
 ...
 * $mobile 手机号
 * $smsTemplate 短信模板
 * $templeContent 验证码 array('code' => $rand)
 */
function sendSMS($mobile, $smsTemplate, $templeContent){
    return True;
}
